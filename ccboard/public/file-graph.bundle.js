...
`
  ...
  var LIGHT_COLORS = {
    ...
    edgeDim: "#e8e8e8",
    pathEdge: "#0066aa90",
    read: { r: 0, g: 120, b: 150 },
    edit: { r: 180, g: 120, b: 0 },
    write: { r: 130, g: 50, b: 170 },
    grep: { r: 0, g: 130, b: 30 },
    glob: { r: 0, g: 130, b: 30 },
    default: { r: 0, g: 120, b: 150 }
  };
  ...
  function heatToColor(heat, actionType) {
    const C = getColors();
    const base = C[actionType] || C.default;
    if (isLightMode()) {
      const gray = 200;
      const r2 = Math.round(gray + (base.r - gray) * heat);
      const g2 = Math.round(gray + (base.g - gray) * heat);
      const b2 = Math.round(gray + (base.b - gray) * heat);
      return `rgb(${r2},${g2},${b2})`;
    }
  ...
  function buildFileGraph(files) {
    ...
    for (const folder of folders) {
      ...
      if (parent && graph.hasNode(parent)) {
        try {
          graph.addEdge(parent, folder, { color: DARK_COLORS.edgeDim, size: 0.5 });
        } catch {
        }
      }
    }
    for (const f of files) {
      ...
      if (parent && graph.hasNode(parent)) {
        try {
          graph.addEdge(parent, f, { color: DARK_COLORS.edgeDim, size: 0.3 });
        } catch {
        }
      }
    }
    import_graphology_layout_forceatlas2.default.assign(graph, {
      iterations: 100,
      settings: { gravity: 1, scalingRatio: 2, barnesHutOptimize: true, strongGravityMode: true }
    });
    import_graphology_layout_noverlap.default.assign(graph, { maxIterations: 50, ratio: 1.5 });
    return graph;
  }
  function initSigma(container, graph) {
    return new Sigma(graph, container, {
      ...
      nodeReducer(node, data) {
        ...
        return {
          ...data,
          color: data.nodeKind === "folder" ? C.folderDim : C.fileDim,
          label: null,
      ...
      edgeReducer(edge, data) {
        const C = getColors();
        if (data.isPathEdge) {
          return { ...data, color: C.pathEdge, size: 1.5 };
        }
        return { ...data, color: C.edgeDim, size: 0.15 };
      }
    });
  }
  var agentPath = [];
  ...
  var TOOL_LIGHT = {
    Read: { bg: "#e8f4f8", border: "#0088aa", text: "#006688", label: "READ" },
    Edit: { bg: "#fdf3e0", border: "#b87800", text: "#8a5a00", label: "EDIT" },
    Write: { bg: "#f3e8f8", border: "#7733aa", text: "#6622aa", label: "WRITE" },
    Grep: { bg: "#e8f8e8", border: "#0a8a2a", text: "#0a7a22", label: "GREP" },
    Glob: { bg: "#e8f8e8", border: "#0a8a2a", text: "#0a7a22", label: "GLOB" }
  };
  function getToolColors() {
    return isLightMode() ? TOOL_LIGHT : TOOL_DARK;
  }
  function getToolStyle(tool) {
    const palette = getToolColors();
    return palette[tool] || { bg: "var(--bg)", border: "var(--border)", text: "var(--text)", label: tool };
  }
  function setupNodeClick(sigma, graph, container) {
    ...
    `;
      ...
      modal.style.cssText = `
      background: var(--bg-card); border: 1px solid var(--border); width: 700px; max-width: 95vw;
      max-height: 85vh; display: flex; flex-direction: column; overflow: hidden;
      font-family: 'Share Tech Mono', monospace; font-size: 12px; color: var(--text);
    `;
      const label = node.split("/").pop();
      let html = "";
      html += `<div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
      <div>
        <span style="color:var(--text-bright);font-size:14px;font-weight:500;">${esc(label)}</span>
        <span style="color:var(--text-dim);font-size:10px;margin-left:8px;">${esc(node)}</span>
      </div>
      <span id="node-popup-close" style="cursor:pointer;color:var(--text-dim);padding:4px 10px;border:1px solid var(--border);font-size:11px;">ESC</span>
    </div>`;
      html += `<div style="padding:8px 16px;border-bottom:1px solid #111;display:flex;gap:12px;flex-shrink:0;">
      <span style="font-size:9px;color:var(--text-dim);letter-spacing:1px;">LEGEND:</span>
      <span style="font-size:9px;color:${getToolStyle("Read").text};">\u25CF READ</span>
      <span style="font-size:9px;color:${getToolStyle("Edit").text};">\u25CF EDIT</span>
      <span style="font-size:9px;color:${getToolStyle("Write").text};">\u25CF WRITE</span>
      <span style="font-size:9px;color:${getToolStyle("Grep").text};">\u25CF GREP</span>
    </div>`;
      html += `<div style="padding:10px 16px;border-bottom:1px solid #111;flex-shrink:0;">
      <div style="font-size:9px;color:var(--text-dim);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;">AGENT PATH \u2014 ${agentPath.length} steps</div>
      ...
      for (let pi = 0; pi < agentPath.length; pi++) {
        ...
        const bg = isThis ? ts.bg : "var(--bg)";
        const color = isThis ? "var(--text-bright)" : ts.text;
        const border = isThis ? `2px solid ${ts.border}` : `1px solid var(--border)`;
      ...
      ">`;
        ...
        html += `${esc(name)}`;
        if (pi < agentPath.length - 1) html += `</span><span style="color:var(--text-dim);font-size:8px;">\u2192</span>`;
        else html += `</span>`;
      }
      html += `</div></div>`;
      html += `<div style="flex:1;overflow-y:auto;padding:12px 16px;">
      <div style="font-size:9px;color:var(--text-dim);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;">ACTIONS ON THIS FILE \u2014 ${actions.length} operations</div>`;
      for (const a of actions) {
        ...
        <div style="display:flex;gap:8px;align-items:baseline;margin-bottom:4px;">
          ...
          <span style="font-size:9px;color:var(--text-dim);">step ${a.index + 1} of ${agentPath.length}</span>
        </div>`;
        const light = isLightMode();
        const delBg = light ? "#fde8e8" : "#1a0808";
        const delColor = light ? "#992222" : "#cc6666";
        const delBorder = light ? "#cc4444" : "#662222";
        const addBg = light ? "#e8fde8" : "#081a08";
        const addColor = light ? "#227722" : "#66cc66";
        const addBorder = light ? "#44aa44" : "#226622";
        const mutedText = light ? "var(--text)" : "#666";
        if (a.tool === "Edit" && (a.oldString || a.newString)) {
          html += `<div style="margin-top:4px;font-family:'JetBrains Mono','Share Tech Mono',monospace;">`;
          if (a.oldString) {
            html += `<div style="background:${delBg};padding:6px 10px;font-size:11px;color:${delColor};white-space:pre-wrap;word-break:break-all;max-height:120px;overflow-y:auto;border-left:2px solid ${delBorder};margin-bottom:2px;">- ${esc(a.oldString)}</div>`;
          }
          if (a.newString) {
            html += `<div style="background:${addBg};padding:6px 10px;font-size:11px;color:${addColor};white-space:pre-wrap;word-break:break-all;max-height:120px;overflow-y:auto;border-left:2px solid ${addBorder};">+ ${esc(a.newString)}</div>`;
          }
          html += `</div>`;
        } else if (a.tool === "Write" && a.newString) {
          html += `<div style="margin-top:4px;font-family:'JetBrains Mono','Share Tech Mono',monospace;">
          <div style="background:${addBg};padding:6px 10px;font-size:11px;color:${addColor};white-space:pre-wrap;word-break:break-all;max-height:150px;overflow-y:auto;border-left:2px solid ${addBorder};">+ ${esc(a.newString)}</div>
        </div>`;
        } else if (a.tool === "Write") {
          html += `<div style="font-size:10px;color:${mutedText};margin-top:2px;">new file created</div>`;
        } else if (a.tool === "Read") {
          html += `<div style="font-size:10px;color:${mutedText};margin-top:2px;">file contents read into context</div>`;
        } else if (a.tool === "Grep") {
