...
function heatToColor(heat, actionType) {
  ...
  if (isLightMode()) {
    // In light mode: interpolate from gray toward the color
    const gray = 200;
    const r = Math.round(gray + (base.r - gray) * heat);
    const g = Math.round(gray + (base.g - gray) * heat);
    const b = Math.round(gray + (base.b - gray) * heat);
    return `rgb(${r},${g},${b})`;
  }
...
function buildFileGraph(files) {
  ...
  for (const folder of folders) {
    ...
    if (parent && graph.hasNode(parent)) {
      try { graph.addEdge(parent, folder, { color: DARK_COLORS.edgeDim, size: 0.5 }); } catch {}
    }
  }
  ...
  for (const f of files) {
    ...
    if (parent && graph.hasNode(parent)) {
      try { graph.addEdge(parent, f, { color: DARK_COLORS.edgeDim, size: 0.3 }); } catch {}
    }
  }
...
// Color for each action type — theme-aware
const TOOL_DARK = {
...
function getToolColors() { return isLightMode() ? TOOL_LIGHT : TOOL_DARK; }
